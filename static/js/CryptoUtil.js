 ...
 */
function base64UrlSafeToBase64(val) {
  return val.replace(new RegExp('_', 'g'), '/').replace(new RegExp('-', 'g'), '+');
};
 ...
 */
 function binToStr(val) {
  return btoa(new Uint8Array(val).reduce((s, byte) => s + String.fromCharCode(byte), ''));
};
 ...
 */
function strToBin(val) {
  return Uint8Array.from(atob(this.base64UrlSafeToBase64(val)), c => c.charCodeAt(0));
};
